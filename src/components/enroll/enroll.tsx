...
import { PasskeyEnroll } from "./screens/passkey-enroll";
...
import { EnrollContext } from "./use-enroll-context";
...
export function Enroll({
  onSuccess,
  onCancel,
  enrollOptions: { token, user, verificationMethods },
  onTokenExpired,
}: EnrollProps) {
  ...
  // the useCallback for `handleSuccess`. `handleSuccess`
  ...
  const handleSuccess = React.useCallback(({ token }: { token: string }) => {
    setOpen(false);
